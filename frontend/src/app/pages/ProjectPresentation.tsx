...
import { ArrowLeft, ChevronLeft, ChevronRight, ClipboardList, Download, FileText, Landmark, Target, Trophy, Users, AlertTriangle } from "lucide-react";
...
interface PresentationData {
  ...
  beneficiariesSummary: { totalBeneficiaries: number; byCategory: Array<{ label: string; value: number }> };
...
export function ProjectPresentation({ projectId }: { projectId: string }) {
  ...
  const goToSlide = useCallback((next: number) => setSlide(Math.max(0, Math.min(totalSlides - 1, next))), []);
  ...
  const slides = [
    ...
    { title: "Project Objectives", eyebrow: "Purpose", content: <div className="max-w-4xl"><p className="text-2xl leading-9 text-slate-700">{data.project.objectives || data.project.description || "No objectives recorded yet."}</p></div> },
    ...
    { title: "Beneficiaries Summary", eyebrow: "Reach", content: <div className="grid gap-8 lg:grid-cols-[0.7fr_1.3fr]"><MetricCard icon={Users} label="Total Beneficiaries" value={data.beneficiariesSummary.totalBeneficiaries} /><div className="h-80">{data.beneficiariesSummary.byCategory.length ? <ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={data.beneficiariesSummary.byCategory} dataKey="value" nameKey="label" outerRadius={110} label>{data.beneficiariesSummary.byCategory.map((_, index) => <Cell key={index} fill={["#0e7490", "#7c3aed", "#16a34a", "#ca8a04"][index % 4]} />)}</Pie><Tooltip /></PieChart></ResponsiveContainer> : <EmptyLine>No beneficiary records available.</EmptyLine>}</div></div> },
