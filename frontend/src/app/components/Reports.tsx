...
export function Reports() {
  ...
  const { data: beneficiaries, loading: beneficiariesLoading, error: beneficiariesError } = useProjectBeneficiaries();
  const { data: reports, loading: reportsLoading, error: reportsError, refresh: refreshReports } = useProjectReports();
  const [generating, setGenerating] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const loading = financialLoading || challengesLoading || beneficiariesLoading || reportsLoading;
  const loadError = financialError || challengesError || beneficiariesError || reportsError;
  ...
  async function generate() {
    ...
    try {
      ...
      await Promise.all([refresh(), refreshReports()]); window.open(result.url, "_blank", "noopener,noreferrer");
