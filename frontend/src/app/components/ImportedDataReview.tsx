          ...
          {showProject && draft && (
            <section className="rounded-lg border border-border bg-card p-4 shadow-sm">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <Field label="Project code" value={text(draft.project.project_code)} onChange={(value) => updateProject({ project_code: emptyToNull(value) })} />
                <Field label="Project manager" value={text(draft.project.project_manager)} onChange={(value) => updateProject({ project_manager: emptyToNull(value) })} />
                <Field label="Planned start date" type="date" value={text(draft.project.planned_start_date)} onChange={(value) => updateProject({ planned_start_date: emptyToNull(value) })} />
                <Field label="Actual start date" type="date" value={text(draft.project.actual_start_date)} onChange={(value) => updateProject({ actual_start_date: emptyToNull(value) })} />
                <Field label="Planned completion date" type="date" value={text(draft.project.planned_completion_date)} onChange={(value) => updateProject({ planned_completion_date: emptyToNull(value) })} />
