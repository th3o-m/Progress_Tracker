    ...
    <div className="overflow-hidden rounded-md border border-border bg-card shadow-sm">
      ...
      <form onSubmit={submit} className="space-y-5 p-6">
        {tab === "activity" && <div className="grid grid-cols-2 gap-4">
          ...
          <label className="text-sm">Responsible officer *<select required value={form.responsibleOfficer} onChange={(e) => set("responsibleOfficer", e.target.value)} className={input}><option value="">Select member...</option>{members.filter((m) => m.profiles?.active).map((m) => <option key={m.id} value={m.profiles!.id}>{m.profiles!.full_name} ({m.role})</option>)}</select></label>
