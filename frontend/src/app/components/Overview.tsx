...
export function Overview() {
  ...
  const unresolved = challenges.filter((item) => !item.resolved).length;
  ...
  const cards = [
    ...
    { label: "Open Challenges", value: unresolved, icon: AlertTriangle, color: "text-red-700", bg: "bg-red-50" },
  ...
  return <div className="space-y-6">
    ...
    <div className="grid grid-cols-5 gap-4">{cards.map((card) => <div key={card.label} className="rounded-md border border-border bg-card p-4 shadow-sm"><div className={`mb-3 inline-flex rounded-md p-2 ${card.bg}`}><card.icon className={`h-5 w-5 ${card.color}`} /></div><p className="text-2xl font-bold">{card.value}</p><p className="text-xs text-muted-foreground">{card.label}</p></div>)}</div>
